...
Copy-Item "$SkillsSrc\*.md" -Destination $ClaudeSkills -Force
Get-ChildItem "$SkillsSrc\*.md" | ForEach-Object { Write-Host "      已安装: $($_.Name)" }
...
Copy-Item "$SkillsSrc\*.md" -Destination $CodexSkills -Force
Get-ChildItem "$SkillsSrc\*.md" | ForEach-Object { Write-Host "      已安装: $($_.Name)" }
